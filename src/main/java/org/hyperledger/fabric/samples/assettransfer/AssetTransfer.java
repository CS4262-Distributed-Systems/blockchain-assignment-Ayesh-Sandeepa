...
public final class AssetTransfer implements ContractInterface {
    ...
    public Asset DuplicateAsset(final Context ctx, final String assetID, final String newOwner) {
        ...
        Asset newAsset = new Asset(asset.getAssetID().concat("_duplicate"), asset.getColor(), asset.getSize(), newOwner, asset.getAppraisedValue());
        //Use a Genson to conver the Asset into string, sort it alphabetically and serialize it into a json string
        String json1 = genson.serialize(asset);
        stub.putStringState(assetID, json1);
        String sortedJson = genson.serialize(newAsset);
        stub.putStringState(assetID.concat("_duplicate"), sortedJson);
    ...
    /*
    @Transaction(intent = Transaction.TYPE.SUBMIT)
    public Asset CopyAsset(final Context ctx, final String assetID, final String secondOwner){
        ChaincodeStub stub=ctx.getStub();
        String assetJson=stub.getStringState(assetID);
        ...
        String json2=genson.serialize(json2);
        stub.putStringState(assetID, json2);
